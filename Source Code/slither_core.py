...
# from crytic_compile import CryticCompile how to remove the error for the code crytic compile could not be resolved? 
...
class SlitherCore(Context):
    ...
    def get_contract_from_name(self, contract_name: Union[str, Constant]) -> List[Contract]:
        ...
        contracts = []
        for compilation_unit in self._compilation_units:
            #Analyze contract_name to see if it is an alias
            if contract_name in compilation_unit.aliases:
                contract_name = compilation_unit.aliases[contract_name]
            contracts += compilation_unit.get_contract_from_name(contract_name)
        return contracts
